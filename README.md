...
One binary, `recall`, three faces:

- **MCP server** — `recall mcp` (stdio): `store_memory`, `search_memory`, `get_memory`, `rate_memory`, `recall` in any MCP-capable harness.
- **CLI** — for agents that shell out: JSON output when piped, exit code 0/1 with machine-readable errors, never a prompt.
- **TUI** — `recall tui`, for humans: browse and live-search, read, rate, prune (delete is TUI-only — agents supersede), correct via a pre-filled supersede editor, review downvoted candidates (`R`), and manage the remote endpoint — all against local or hosted memory alike.
...
# the loop agents run:
recall pack -k project:myapp -k lint            # session bootstrap: ranked context block
recall search -q "golangci config" -k go        # or browse ranked summaries
recall get <id>                                 # full content (counts as usage)
recall rate <id> up                             # feedback drives future ranking
recall store --summary "one line" --kind research -k go -k lint "the finding..."
recall store --supersedes <old-id> ...          # corrections replace, never edit
```
...
recall serve --addr :7846                       # data in ~/.local/share/recall/server
recall keys create --space team-x --name ivan   # prints the key exactly once
recall keys revoke <id>                         # one teammate out, others unaffected
recall spaces export team-x ./team-x.db         # live-safe snapshot = a local memory.db
...
Stored memories' `source` becomes the key's name server-side, so shared writes attribute themselves. Migrate-to-local is `spaces export` + `RECALL_DB=<file>`; migrate-to-hosted is copying a local `memory.db` into `spaces/<name>.db`.
...
- **Concurrent by design**: a harness-connected `recall mcp` server and CLI calls (yours, subagents', other harnesses') safely share the file — WAL + immediate write transactions underneath. Same machine only; never put the file on NFS.
