...
Local-first: the memory **is one `.db` file**, no daemon required. Optionally hosted: the same core behind a gRPC server with API keys, accessed by the same CLI/TUI/MCP faces. Working name: repo `go-agent-memory`, binary **`recall`**.
...
**Recall pack:** `pack(keywords, budget)` — the one-call version: assembles the top-ranked memories into a single markdown block trimmed to a size budget, ready to drop into context. This is the tool agents will actually reach for at session start.
...
**Teach the agents:** `recall prompt` prints a recommended instructions block (when to search, how to store, keyword conventions, rating etiquette) for pasting into a harness's AGENTS.md/CLAUDE.md — the system documents its own usage contract.
...
**One SQLite file per space** (`<server-dir>/spaces/<name>.db`, names locked to `[a-z0-9-]+`): isolation by construction — no space column, no WHERE-clause to forget — and every space file is byte-identical to a local `memory.db`. Migrate-to-local is `recall spaces export` (live-safe `VACUUM INTO`) + `RECALL_DB=<file>`; migrate-to-hosted is copying a local file in. A lazy `SpaceRegistry` opens spaces on first use; `keys.db` (own migration set) is the control plane.

**Client side:** `recall remote set <addr> <key>` (verified before saving; config at `~/.config/recall/remote.json`, 0600; `RECALL_REMOTE_ADDR`/`RECALL_API_KEY` override per-field) swaps the local Service for the gRPC client inside `connect()` — every face (CLI, MCP, TUI) follows automatically, and typed domain errors survive the wire. Transport is plaintext: private network or TLS-terminating proxy.
...
4. Default DB location: global `~/.local/share/recall/memory.db` with `--db`/env override (proposal), or per-project files?
